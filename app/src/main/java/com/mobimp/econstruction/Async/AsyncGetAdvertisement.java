...
public class AsyncGetAdvertisement extends AsyncTask<String, String, Boolean> {
	String ServerUrl;
	private ProgressDialog dialog;
	Context context;
	GetAdvertiseTask Task;
	String mInfo;
    List<AdvertisementItem> mArray;
	public AsyncGetAdvertisement(Context c, String Url, GetAdvertiseTask task) {
		super();
		ServerUrl = Url;
	...
	protected void onPostExecute(Boolean result) {
		...
		if (result) {
			...
			Task.GetAdvertiseTaskSuccess(mArray);
			
			 
		} else {
			
			Task.GetAdvertiseTaskFailed(mInfo);
		}
	}
	...
	protected Boolean doInBackground(String... params) {
		...
		if (jsonStr != null) {
			try {
				...
				{
                    List<AdvertisementItem> mCatArray=new ArrayList<AdvertisementItem>();
					JSONArray CategoryArray = jsonObj.getJSONArray("output");
					for(int i=0;i<CategoryArray.length();i++){
						JSONObject cat = CategoryArray.optJSONObject(i);
						AdvertisementItem mItem=new AdvertisementItem();
						mItem.title=cat.optString("Title");
						mItem.Desc=cat.optString("Description");
	...
	public interface GetAdvertiseTask {
		void GetAdvertiseTaskSuccess(List<AdvertisementItem> mArray);

		void GetAdvertiseTaskFailed(String info);
	}
}
